...
use RestAPI\RouterPythonService as Router;
...
class RestPythonServiceClient
{
     ...
     * @var string
     */
    private static $apiVersion = 'python';
    ...
    private static function assertInitialized()
    {
        if (empty(self::$sysId) &&
            ...
            throw new \RuntimeException(
                'RestPythonServiceClient is not initialized, ' .
                'please specify application key ' .
                'with RestPythonServiceClient::initialize.'
            );
        }
    }
    ...
    private static function log($unionid, $key, $data)
    {
        ...
        __LOG_MESSAGE($data, "RestPythonServiceClient($unionid)::$key");
    }
